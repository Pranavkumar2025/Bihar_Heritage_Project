...
const Header2 = () => {
  ...
  const navLinks = [
    { name: "Home", to: "/" },
    { name: "About Us", to: "/#footer-section" },
    { name: "Excavation", to: "/#excavation-section" },
    { name: "Exploration", to: "/#explore-join-section" },
              ...
              {navLinks.map(({ name, to, subMenu }, idx) => (
                ...
                >
                  {!subMenu ? (
                    <NavLink
                      to={to}
                      onClick={() => setActiveDropdown(null)} // Close dropdown when a top-level link is clicked
                      className={({ isActive }) => {
                        let trulyActive = false;
                        const linkHash = to.includes("#") ? to.substring(to.indexOf("#")) : "";
                        if (linkHash) {
                          trulyActive = isActive && location.hash === linkHash;
                        } else {
                          trulyActive = isActive && (location.hash === "" || location.hash === "#");
                        }
                          ...
                          >
                            {subMenu.map((item) => (
                              <NavLink
                                key={item.name}
                                to={item.to}
                                onClick={() => setActiveDropdown(null)} // Close dropdown on submenu item click
                                className={({ isActive }) => // Standard isActive is fine for page links in dropdown
                    ...
                    <div key={name} className="w-full text-center">
                      {!subMenu ? (
                        <NavLink
                          ...
                          onClick={() => {
                            setMenuOpen(false);
                            setActiveDropdown(null); // Close mobile menu and any active dropdown
                          }}
                          className={({ isActive }) => {
                            let trulyActive = false;
                            const linkHash = to.includes("#") ? to.substring(to.indexOf("#")) : "";
                            ...
                            return `block py-2 text-base font-medium ${
                              trulyActive ? "text-yellow-400" : "hover:text-yellow-400"
                            }`;
                          }}
                        >
                          {name}
                              ...
                              >
                                {subMenu.map((item) => (
                                  <NavLink
                                    ...
                                    className={({ isActive }) => // Standard isActive for page links
