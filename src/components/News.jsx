...
const newsItems = [
  ...
  },
  {
    ...
    icon: <CalendarDays className="w-6 h-6 text-yellow-400" />,
  },
];
...
const News = () => {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden py-24 px-6 md:px-20 bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800 text-white font-[Poppins]">
      {/* Zigzag SVG */}
      <div className="absolute top-0 left-0 w-full overflow-hidden leading-none rotate-180 z-0 opacity-10">
        <svg
      ...
      >
        <h2 className="text-4xl md:text-6xl font-extrabold text-yellow-500 mb-6 uppercase">
        ...
        {newsItems.map((item, index) => (
          <SwiperSlide key={index} className="flex justify-center">
            <motion.div
              ...
              className="bg-gray-800 w-full max-w-sm rounded-2xl border border-gray-700 shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 p-6"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="bg-gray-900 p-3 rounded-full shadow">{item.icon}</div>
                <h3 className="text-xl font-bold text-white">{item.title}</h3>
              </div>
              <p className="text-sm text-gray-400 mb-2 italic">{item.date}</p>
              <p className="text-[15px] text-gray-300 leading-relaxed">{item.description}</p>
            </motion.div>
          </SwiperSlide>
        ))}
      </Swiper>
